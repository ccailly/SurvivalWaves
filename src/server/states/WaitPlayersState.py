...
class WaitPlayersState(AbstractState):
    ...
    def process(self) -> None:
        if self.checks():
            ...
            self._referee.ruleArena('info', 'Game is running in {}s'.format(max(0, round(10 - (time.time() - self._timer)))))
    ...
    def next(self) -> IState:
        ...
        """
        return self if self.checks() or (hasattr(self, "_timer") and time.time() - self._timer < 10) else StartGameState(self.getReferee())
